...
import { Key, Plus, Trash2, Copy, Check, ExternalLink, AlertCircle, CheckCircle2, X, Shield, ShieldAlert, Calendar } from 'lucide-react'
...
export default function UserLicensesContent() {
  ...
  const [spamStats, setSpamStats] = useState<SpamStats>({ totalBlocked: 0, blockedToday: 0, blockedThisWeek: 0, topBlockReason: null })
      ...
      {licenses.length > 0 && (
        <>
          ...
          <Card className="border-[#22D6DD]/30 bg-gradient-to-r from-[#22D6DD]/5 to-transparent">
            ...
            <CardContent>
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center p-4 bg-white rounded-[9px] border border-[#d9dde1]">
                  <ShieldAlert className="h-6 w-6 mx-auto mb-2 text-[#22D6DD]" />
                  <p className="text-xs text-slate-500 mb-1">Heute blockiert</p>
                  <p className="text-2xl font-bold text-[#22D6DD]">{spamStats.blockedToday.toLocaleString()}</p>
                </div>
                <div className="text-center p-4 bg-white rounded-[9px] border border-[#d9dde1]">
                  <Calendar className="h-6 w-6 mx-auto mb-2 text-[#EC4899]" />
                  <p className="text-xs text-slate-500 mb-1">Diese Woche</p>
                  <p className="text-2xl font-bold text-[#EC4899]">{spamStats.blockedThisWeek.toLocaleString()}</p>
                </div>
                <div className="text-center p-4 bg-white rounded-[9px] border border-[#d9dde1]">
                  <Shield className="h-6 w-6 mx-auto mb-2 text-slate-600" />
                  <p className="text-xs text-slate-500 mb-1">Gesamt blockiert</p>
                  <p className="text-2xl font-bold text-slate-900">{spamStats.totalBlocked.toLocaleString()}</p>
                </div>
              </div>
              {spamStats.topBlockReason && (
                <p className="text-center text-sm text-slate-500 mt-4">
                  Häufigster Blockgrund: <span className="font-medium text-[#22D6DD]">{spamStats.topBlockReason}</span>
                </p>
              )}
            </CardContent>
          </Card>
