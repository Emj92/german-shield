...
(function($) {
    ...
    $(document).ready(function() {
        ...
        // 1. Basis-Checks
        if (typeof germanShieldAdmin === 'undefined') {
            error('germanShieldAdmin Object fehlt!');
            return;
        }
        
        log('germanShieldAdmin vorhanden:', germanShieldAdmin);
        
        // 1.5 Theme Switcher
        var darkMode = localStorage.getItem('germanShield_darkMode') === 'true';
        if (darkMode) {
            $('body').addClass('german-shield-dark-mode');
            $('.theme-icon-light').hide();
            $('.theme-icon-dark').show();
        }
        
        $('#german-shield-theme-toggle').on('click', function() {
            $('body').toggleClass('german-shield-dark-mode');
            var isDark = $('body').hasClass('german-shield-dark-mode');
            localStorage.setItem('germanShield_darkMode', isDark);
        ...
        var currentLang = localStorage.getItem('germanShield_language') || 'de';
        ...
        function translateUI(lang) {
            ...
            $('.german-shield-tab').each(function() {
                var $tab = $(this);
            ...
            $('.german-shield-stat-card p').each(function() {
                var $p = $(this);
            ...
            // Tabellen-Header
            $('.german-shield-table thead th').each(function() {
                var $th = $(this);
            ...
            // Überschriften
            $('.german-shield-recent-blocks h2').each(function() {
                var $h = $(this);
        ...
        $('#german-shield-language-toggle').on('click', function() {
            currentLang = currentLang === 'de' ? 'en' : 'de';
            localStorage.setItem('germanShield_language', currentLang);
        ...
        if (currentLang === 'en') {
            $('#german-shield-language-toggle .language-flag').text('🇬🇧');
            $('#german-shield-language-toggle .language-code').text('EN');
            setTimeout(function() {
                translateUI('en');
            }, 100);
        }
        ...
        // Bindet an ALLE Toggles, inkl. GEO, aber NICHT Länder (da diese keine .german-shield-toggle Klasse haben)
        log('Binde Toggle-Handler...');
        var toggleCount = $('.german-shield-toggle input[type="checkbox"]').length;
        log('Gefundene Toggles:', toggleCount);
        
        // DIREKTE Bindung an ALLE Toggles (nicht Delegation!)
        $('.german-shield-toggle input[type="checkbox"]').each(function() {
            var $toggle = $(this);
        ...
        function handleToggleUI(name, isChecked) {
            ...
            switch(name) {
                ...
                case 'honeypot_enabled':
                    $target = $('#honeypot-settings');
                    break;
                case 'timestamp_enabled':
                    $target = $('#timestamp-settings');
                    break;
        ...
        $('.german-shield-tab').on('click', function(e) {
            e.preventDefault();
            try {
                ...
                // Buttons umschalten
                $('.german-shield-tab').removeClass('active');
                $(this).addClass('active');
                
                // Content umschalten
                $('.german-shield-tab-content').hide(); // Sicherer als removeClass
                $('#tab-' + tabId).show(); // Sicherer als addClass
                
                // Speichern (außer GEO)
                if (window.localStorage && tabId !== 'geo') {
                    localStorage.setItem('german_shield_active_tab', tabId);
                } else if (window.localStorage) {
                    localStorage.removeItem('german_shield_active_tab');
                }
        ...
        if (window.localStorage) {
            var savedTab = localStorage.getItem('german_shield_active_tab');
            if (savedTab && savedTab !== 'geo') {
                var $tabBtn = $('.german-shield-tab[data-tab="' + savedTab + '"]');
                if ($tabBtn.length) {
        ...
        $('#add-phrase').on('click', function() {
            var tmpl = '<div class="phrase-item"><input type="text" name="blocked_phrases[]" placeholder="Phrase..."><button type="button" class="german-shield-btn-danger remove-phrase"><span class="dashicons dashicons-trash"></span></button></div>';
        ...
        $('#clear-debug-log').on('click', function() {
            ...
            $.post({
                url: germanShieldAdmin.ajaxUrl,
                data: { 
                    action: 'german_shield_clear_log', 
                    nonce: germanShieldAdmin.nonce 
                },
                success: function() {
                    alert('Log geleert.');
        ...
        $('.view-details-btn').on('click', function() {
            ...
            // Modal erstellen
            var modalHtml = '<div class="german-shield-modal-overlay" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.7); z-index: 99999; display: flex; align-items: center; justify-content: center;">' +
                '<div class="german-shield-modal" style="background: #fff; border-radius: 12px; max-width: 700px; width: 90%; padding: 30px; box-shadow: 0 20px 60px rgba(0,0,0,0.3); max-height: 90vh; overflow-y: auto;">' +
            ...
            // Modal schließen
            $('.modal-close-btn, .german-shield-modal-overlay').on('click', function(e) {
                if (e.target === this) {
                    $('.german-shield-modal-overlay').fadeOut(200, function() {
                        $(this).remove();
                    });
                }
            });
        ...
        $('#register-free-btn').on('click', function() {
            ...
            $.post({
                url: germanShieldAdmin.ajaxUrl,
                data: {
                    action: 'german_shield_register_free',
                    nonce: germanShieldAdmin.nonce,
                    email: email
                },
                success: function(response) {
        ...
        function updateBadgePreview() {
            ...
            if (customImage && customImage.trim() !== '') {
                ...
                $('#badge-icon').html('<img src="' + germanShieldAdmin.pluginUrl + 'assets/images/logo_klein.png" alt="German Shield" style="width: 24px; height: 24px; object-fit: contain;">');
            }
        }
        ...
        $('.german-shield-free-tab').on('click', function() {
            $('.german-shield-free-tab').removeClass('active').css({
        ...
        $('#activate-free-key-btn').on('click', function() {
            ...
            $.ajax({
                ...
                method: 'POST',
                data: {
                    action: 'german_shield_activate_free_key',
                    nonce: germanShieldAdmin.nonce,
                    license_key: key
                },
                success: function(response) {
        ...
        $('.german-shield-free-tab.active').css({
            'border-bottom-color': '#22D6DD',
            'color': '#22D6DD'
        });
        ...
        $('#premium-license-form').on('submit', function(e) {
            ...
            $.ajax({
                ...
                method: 'POST',
                data: {
                    action: 'german_shield_activate_premium',
                    nonce: germanShieldAdmin.nonce,
        ...
        $(document).on('click', '.regenerate-honeypot-btn', function() {
            ...
            setTimeout(function() {
                ...
                // Auto-Save
                performAutoSave('honeypot_fields[]', newName);
            }, 300);
        });
