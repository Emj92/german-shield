...
class German_Shield {
    ...
    private function load_dependencies() {
        $this->admin = new German_Shield_Admin();
        $this->antispam = new German_Shield_AntiSpam();
        $this->geo_blocking = new German_Shield_GeoBlocking();
        $this->phrase_blocking = new German_Shield_PhraseBlocking();
        $this->statistics = new German_Shield_Statistics();
        $this->form_detector = new German_Shield_FormDetector();
    ...
    public function run() {
        German_Shield_Logger::log_hook('run() wird aufgerufen');
        
        // Initialize admin interface
        add_action('admin_menu', array($this->admin, 'add_admin_menu'));
        add_action('admin_enqueue_scripts', array($this->admin, 'enqueue_admin_assets'));
        German_Shield_Logger::log_hook('Admin-Hooks registriert');
        
        // Initialize form protection
        add_action('wp_enqueue_scripts', array($this->antispam, 'enqueue_frontend_scripts'));
        German_Shield_Logger::log_hook('Frontend-Scripts-Hook registriert');
        ...
        add_action('wp_ajax_nopriv_german_shield_validate', array($this, 'ajax_validate'));
        add_action('wp_ajax_german_shield_validate', array($this, 'ajax_validate'));
        German_Shield_Logger::log_hook('Validierungs-Hooks registriert');
        
        // Generic form hook
        add_action('wp_loaded', array($this->form_detector, 'detect_and_protect_forms'));
        German_Shield_Logger::log_hook('Form-Detector-Hook registriert');
        
        // Initialize AJAX Handler
        new German_Shield_Ajax();
        German_Shield_Logger::log_hook('AJAX-Handler initialisiert');
    }
    ...
    public function perform_validation($data) {
        ...
        German_Shield_Logger::log('[VALIDATION] 🔍 perform_validation() aufgerufen - IP: ' . $ip . ', POST-Keys: ' . implode(', ', array_keys($data)));
        ...
        // Nonce Check
        $t = microtime(true);
        if (!isset($data['gs_nonce']) || !wp_verify_nonce($data['gs_nonce'], 'german_shield_nonce')) {
    ...
    private function log_timing($name, $start_time) {
        ...
        do_action('qm/lap', 'german-shield/' . $name, $duration);
        
        // Debug-Log
        German_Shield_Logger::log('[TIMING] ' . $name . ': ' . number_format($duration, 2) . 'ms');
    }
}
