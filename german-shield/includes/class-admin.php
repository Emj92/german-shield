...
class German_Shield_Admin {
    
    public function __construct() {
        // AJAX Handler für Log-Löschen
        add_action('wp_ajax_german_shield_clear_log', array($this, 'ajax_clear_log'));
    }
    ...
    public function add_admin_menu() {
        German_Shield_Logger::log_admin('add_admin_menu() wird aufgerufen');
        
        // Hauptmenü mit eigenem Logo
        $icon_url = GERMAN_SHIELD_PLUGIN_URL . 'assets/images/logo_klein.png';
        add_menu_page(
            'German Shield',
            'German Shield',
            'manage_options',
        ...
        German_Shield_Logger::log_admin('Admin-Menü erfolgreich registriert');
    }
    
    public function add_menu_icon_css() {
        ?>
        <style>
            /* German Shield Logo im Menü skalieren */
            #adminmenu #toplevel_page_german-shield .wp-menu-image img {
            ...
            /* Hover-Effekt */
            #adminmenu #toplevel_page_german-shield:hover .wp-menu-image img {
                opacity: 1 !important;
            }
            
            /* Menüpunkt normale Höhe - KEIN extra Platz */
            #adminmenu #toplevel_page_german-shield {
                margin-bottom: 0 !important;
            }
            
            /* Submenü standardmäßig versteckt */
            #adminmenu #toplevel_page_german-shield .wp-submenu {
            ...
            #adminmenu #toplevel_page_german-shield:hover .wp-submenu,
            #adminmenu #toplevel_page_german-shield .wp-submenu:hover {
                display: block !important;
            }
            
            /* Hover-Bereich erweitern */
            #adminmenu #toplevel_page_german-shield .wp-submenu::before {
                content: '';
                position: absolute;
                left: -50px;
                top: 0;
            ...
            #adminmenu #toplevel_page_german-shield .wp-submenu li a {
                padding: 10px 15px !important;
                color: #c3c4c7 !important;
                transition: all 0.2s ease !important;
            }
            
            #adminmenu #toplevel_page_german-shield .wp-submenu li a:hover {
    ...
    public function enqueue_admin_assets($hook) {
        if ($hook !== 'toplevel_page_german-shield') {
            return;
        }
        
        German_Shield_Logger::log_admin('enqueue_admin_assets() wird aufgerufen');
        
        wp_enqueue_style(
            'german-shield-admin',
            GERMAN_SHIELD_PLUGIN_URL . 'assets/css/admin.css',
            array(),
            GERMAN_SHIELD_VERSION
        );
        
        wp_enqueue_script(
            'german-shield-admin',
            GERMAN_SHIELD_PLUGIN_URL . 'assets/js/admin.js',
            array('jquery'),
            GERMAN_SHIELD_VERSION . '-' . time(), // Cache-Buster!
            true
        );
        
        wp_localize_script('german-shield-admin', 'germanShieldAdmin', array(
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('german_shield_admin'),
            'pluginUrl' => GERMAN_SHIELD_PLUGIN_URL,
        ));
        
        German_Shield_Logger::log_admin('Admin-Assets geladen (JS aktiviert mit Cache-Buster)');
    }
    
    public function render_admin_page() {
        German_Shield_Logger::log_admin('render_admin_page() wird aufgerufen');
        ...
        if (!empty($_POST)) {
            German_Shield_Logger::log_admin('POST-Daten empfangen', array('count' => count($_POST), 'keys' => array_keys($_POST)));
            echo '<div class="notice notice-info" style="margin: 20px 20px 0 0;"><p><strong>German Shield Debug:</strong> POST-Daten empfangen (' . count($_POST) . ' Felder).</p></div>';
        }

        // Speichern verarbeiten (NOCH OHNE Nonce, um Fehler auszuschließen)
        if (isset($_POST['german_shield_save_settings'])) {
            German_Shield_Logger::log_save('Save-Button wurde geklickt');
            $this->save_settings();
            $saved = true;
            German_Shield_Logger::log_save('Einstellungen wurden gespeichert');
        }
        
        $settings = get_option('german_shield_settings', array());
        German_Shield_Logger::log_admin('Einstellungen geladen', array('count' => count($settings)));
        
        $statistics = new German_Shield_Statistics();
        $stats = $statistics->get_stats();
        German_Shield_Logger::log_admin('Statistiken geladen');
        
        $form_stats = new German_Shield_Form_Stats();
        $detected_forms = $form_stats->get_detected_forms();
        German_Shield_Logger::log_admin('Formulare erkannt', array('total' => $detected_forms['total']));
        
        $license_manager = new German_Shield_License();
        $license_info = $license_manager->get_license_info();

        $free_manager = new German_Shield_Free_License();
        ...
        German_Shield_Logger::log_admin('[LICENSE-CHECK] Free aktiv: ' . ($is_free_active ? 'JA' : 'NEIN') . ', Email: ' . $free_email . ', License valid: ' . ($license_info['is_valid'] ? 'JA' : 'NEIN'));
        ...
        if (isset($_GET['show']) && in_array($_GET['show'], array('agb', 'datenschutz', 'impressum'))) {
            ...
            $legal_file = GERMAN_SHIELD_PLUGIN_DIR . 'includes/legal/' . $legal_page . '.php';
        ...
        <div class="german-shield-wrapper">
            <div class="german-shield-header">
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <h1 style="color: #1d2327; margin: 0;">
                        <img src="<?php echo GERMAN_SHIELD_PLUGIN_URL; ?>assets/images/logo_groß.png" alt="German Shield" class="german-shield-logo-img" style="height: 60px; width: auto; margin-right: 15px;">
                        German Shield
                        <span class="german-shield-version">v<?php echo GERMAN_SHIELD_VERSION; ?></span>
                    </h1>
                    
                    <!-- Theme & Language Switcher -->
                    <div class="german-shield-header-controls" style="display: flex; gap: 15px; align-items: center;">
                        <!-- Theme Switcher -->
                        <button id="german-shield-theme-toggle" class="header-control-btn" title="Theme wechseln">
                            <span class="theme-icon-light" style="font-size: 24px;">☀️</span>
                            <span class="theme-icon-dark" style="font-size: 24px; display: none;">🌙</span>
                        </button>
                        
                        <!-- Language Switcher -->
                        <button id="german-shield-language-toggle" class="header-control-btn" title="Sprache wechseln">
                    ...
                    <div class="german-shield-quote-of-day">
            ...
            <div class="german-shield-tabs">
                <button class="german-shield-tab <?php echo $active_tab === 'dashboard' ? 'active' : ''; ?> <?php echo (!$is_free_active && !$license_info['is_valid']) ? 'disabled' : ''; ?>" data-tab="dashboard" <?php echo (!$is_free_active && !$license_info['is_valid']) ? 'disabled' : ''; ?>>
                    <span class="dashicons dashicons-dashboard"></span>
                    Dashboard
                    <?php if (!$is_free_active && !$license_info['is_valid']): ?><span class="lock-badge">🔒</span><?php endif; ?>
                </button>
                <button class="german-shield-tab <?php echo $active_tab === 'antispam' ? 'active' : ''; ?> <?php echo (!$is_free_active && !$license_info['is_valid']) ? 'disabled' : ''; ?>" data-tab="antispam" <?php echo (!$is_free_active && !$license_info['is_valid']) ? 'disabled' : ''; ?>>
                    <span class="dashicons dashicons-shield-alt"></span>
                    Anti-Spam
                    <?php if (!$is_free_active && !$license_info['is_valid']): ?><span class="lock-badge">🔒</span><?php endif; ?>
                </button>
                <button class="german-shield-tab <?php echo $active_tab === 'geo' ? 'active' : ''; ?> <?php echo !$license_info['is_valid'] ? 'pro-feature' : ''; ?> <?php echo (!$is_free_active && !$license_info['is_valid']) ? 'disabled' : ''; ?>" data-tab="geo" <?php echo (!$is_free_active && !$license_info['is_valid']) ? 'disabled' : ''; ?>>
                ...
                <button class="german-shield-tab <?php echo $active_tab === 'phrases' ? 'active' : ''; ?> <?php echo !$license_info['is_valid'] ? 'pro-feature' : ''; ?> <?php echo (!$is_free_active && !$license_info['is_valid']) ? 'disabled' : ''; ?>" data-tab="phrases" <?php echo (!$is_free_active && !$license_info['is_valid']) ? 'disabled' : ''; ?>>
                ...
                <button class="german-shield-tab <?php echo $active_tab === 'notices' ? 'active' : ''; ?> <?php echo (!$is_free_active && !$license_info['is_valid']) ? 'disabled' : ''; ?>" data-tab="notices" <?php echo (!$is_free_active && !$license_info['is_valid']) ? 'disabled' : ''; ?>>
                    <span class="dashicons dashicons-bell"></span>
                    WordPress Spam
                    <?php if (!$is_free_active && !$license_info['is_valid']): ?><span class="lock-badge">🔒</span><?php endif; ?>
                </button>
                <button class="german-shield-tab <?php echo $active_tab === 'settings' ? 'active' : ''; ?> <?php echo (!$is_free_active && !$license_info['is_valid']) ? 'disabled' : ''; ?>" data-tab="settings" <?php echo (!$is_free_active && !$license_info['is_valid']) ? 'disabled' : ''; ?>>
                    <span class="dashicons dashicons-admin-settings"></span>
                    Einstellungen
                    <?php if (!$is_free_active && !$license_info['is_valid']): ?><span class="lock-badge">🔒</span><?php endif; ?>
                </button>
                <button class="german-shield-tab <?php echo $active_tab === 'license' ? 'active' : ''; ?>" data-tab="license">
                    <span class="dashicons dashicons-admin-network"></span>
                    Lizenz
                </button>
            </div>
            
            <form method="post" action="<?php echo esc_url(admin_url('admin.php?page=german-shield')); ?>">
                <?php wp_nonce_field('german_shield_settings', 'german_shield_nonce'); ?>
                
                <!-- Dashboard Tab -->
                <div class="german-shield-tab-content <?php echo $active_tab === 'dashboard' ? 'active' : ''; ?>" id="tab-dashboard">
                    
                    <?php if (!$is_free_active && !$license_info['is_valid']): ?>
                        <div style="background: linear-gradient(135deg, rgba(239, 68, 68, 0.1) 0%, rgba(240, 98, 146, 0.1) 100%); padding: 40px; border-radius: 8px; border: 2px solid #ef4444; text-align: center; margin: 20px 0;">
                            ...
                            <p style="margin: 0 0 20px 0; color: #1d2327; font-size: 16px;">
                                Bitte verifiziere deine E-Mail oder aktiviere eine Lizenz, um German Shield zu nutzen.
                            </p>
                            <a href="<?php echo admin_url('admin.php?page=german-shield&tab=license'); ?>" class="german-shield-btn-primary">
                                Zur Lizenz-Verwaltung →
                            </a>
                        </div>
                    <?php else: ?>
                    
                    <div class="german-shield-stats-grid">
                        <div class="german-shield-stat-card">
                    ...
                    <div class="german-shield-recent-blocks">
                        ...
                        <table class="german-shield-table">
                ...
                <!-- Anti-Spam Tab -->
                <div class="german-shield-tab-content <?php echo $active_tab === 'antispam' ? 'active' : ''; ?>" id="tab-antispam">
                    <?php if (!$is_free_active && !$license_info['is_valid']): ?>
                    ...
                    <?php else: ?>
                    <div class="german-shield-section">
                        <h2>🛡️ Anti-Spam Methoden</h2>
                        
                        <div class="german-shield-settings-grid">
                            <div class="german-shield-setting">
                                <label class="german-shield-toggle">
                            ...
                            <div class="german-shield-subsetting" id="honeypot-settings" style="<?php echo $settings['honeypot_enabled'] !== '1' ? 'display:none;' : ''; ?>">
                                <h3 style="margin-bottom: 20px;">🍯 Honeypot-Verwaltung</h3>
                                
                                <!-- Anzahl der Honeypots -->
                                ...
                                <div class="setting-row">
                                    ...
                                    <div id="honeypot-fields-list">
                                        ...
                                        // Generiere Standardfelder falls nicht vorhanden
                                        ...
                                        // Zeige nur die Anzahl, die im Slider eingestellt ist
                            ...
                            <div class="german-shield-setting">
                                <label class="german-shield-toggle">
                                    <input type="checkbox" name="typing_speed_check" value="1" <?php checked(!empty($settings['typing_speed_check']) && $settings['typing_speed_check'] === '1'); ?>>
                                    <span class="toggle-slider"></span>
                                </label>
                                <div class="setting-info">
                                    <h3>⌨️ Tippgeschwindigkeits-Analyse</h3>
                                    <p>🆕 Erkennt Bots durch unnatürliche Tippgeschwindigkeit. Bots tippen oft < 20ms zwischen Tasten, Menschen 100-300ms.</p>
                                </div>
                            </div>
                            ...
                            <div class="german-shield-setting">
                                ...
                                <div class="setting-info">
                                    ...
                                    <p>Blockiert automatisierte Bot-Kommentare durch erweiterte Anti-Spam-Prüfungen. Schützt vor Spam-Kommentaren in WordPress, WooCommerce und anderen Kommentar-Systemen.</p>
                                </div>
                            </div>
                        </div>
                        
                        <div class="german-shield-subsetting" id="timestamp-settings" style="<?php echo $settings['timestamp_enabled'] !== '1' ? 'display:none;' : ''; ?>">
                            <h3>Zeitstempel-Einstellungen</h3>
                            <div class="setting-row" style="display: flex; align-items: center; gap: 15px; margin-bottom: 15px;">
                                <label style="min-width: 200px; font-weight: 600; color: #1d2327;">Minimale Zeit (Sekunden):</label>
                                <input type="number" name="timestamp_min" value="<?php echo esc_attr($settings['timestamp_min'] ?? 3); ?>" min="1" max="60" style="width: 100px; padding: 8px; border: 1px solid #c3cbd5; border-radius: 4px;">
                                <span class="description">Mind. Ausfüllzeit</span>
                            </div>
                            <div class="setting-row" style="display: flex; align-items: center; gap: 15px;">
                                <label style="min-width: 200px; font-weight: 600; color: #1d2327;">Maximale Zeit (Sekunden):</label>
                                <input type="number" name="timestamp_max" value="<?php echo esc_attr($settings['timestamp_max'] ?? 3600); ?>" min="60" max="86400" style="width: 100px; padding: 8px; border: 1px solid #c3cbd5; border-radius: 4px;">
                                <span class="description">Max. Formular-Alter</span>
                            </div>
                        </div>
                        
                        <div class="german-shield-setting" style="border: 2px solid #F06292; background: rgba(240, 98, 146, 0.05); padding: 20px; border-radius: 6px; margin-top: 20px;">
                            <label class="german-shield-toggle">
                ...
                <?php
                $geo = new German_Shield_GeoBlocking();
                $geo->render_tab($settings, $license_info);
                ?>
                
                <!-- Phrase Blocking Tab -->
                <div class="german-shield-tab-content <?php echo $active_tab === 'phrases' ? 'active' : ''; ?>" id="tab-phrases">
                    <div class="german-shield-section">
                        ...
                        <div class="german-shield-subsetting" id="phrase-settings" style="<?php echo $settings['phrase_blocking_enabled'] !== '1' ? 'display:none;' : ''; ?>">
                            <h3>Blockierte Phrasen</h3>
                            
                            <!-- Regex-Modus Toggle -->
                            <div style="margin-bottom: 15px; display: flex; align-items: center; gap: 12px;">
                                <label class="german-shield-toggle">
                                    <input type="checkbox" name="phrase_regex_mode" value="1" <?php checked($settings['phrase_regex_mode'] === '1'); ?>>
                                    <span class="toggle-slider"></span>
                                </label>
                                <div>
                            ...
                            <p class="description" style="margin-bottom: 10px;">
                                <span id="phrase-help-normal" style="<?php echo empty($settings['phrase_regex_mode']) ? '' : 'display:none;'; ?>">
                                    Geben Sie Wörter oder Phrasen ein, die blockiert werden sollen. Trennen Sie mehrere Einträge mit <strong>Komma</strong>.
                                </span>
                                <span id="phrase-help-regex" style="<?php echo !empty($settings['phrase_regex_mode']) ? '' : 'display:none;'; ?>">
                            ...
                            <textarea 
                                ...
                                placeholder="<?php echo !empty($settings['phrase_regex_mode']) ? 'z.B.: V.*i.*a.*g.*r.*a' : 'z.B.: casino, viagra, lottery, cheap pills'; ?>"
                            ><?php 
                                $phrases = $settings['blocked_phrases'] ?? array();
                                if (!empty($settings['phrase_regex_mode'])) {
                            ...
                            <div id="phrase-examples-normal" style="margin-top: 10px; <?php echo empty($settings['phrase_regex_mode']) ? '' : 'display:none;'; ?>">
                                <p class="description" style="color: #646970;">
                                    <strong>Beispiele:</strong> <code>spam, viagra, casino, gewinnspiel, lottery</code>
                                </p>
                            </div>
                            
                            <div id="phrase-examples-regex" style="margin-top: 10px; <?php echo !empty($settings['phrase_regex_mode']) ? '' : 'display:none;'; ?>">
                ...
                <!-- WordPress Spam Tab -->
                <div class="german-shield-tab-content <?php echo $active_tab === 'notices' ? 'active' : ''; ?>" id="tab-notices">
                    <div class="german-shield-section">
                        ...
                        <div class="german-shield-settings-grid">
                            <div class="german-shield-setting">
                                <label class="german-shield-toggle">
                                    <input type="checkbox" name="block_admin_notices" value="1" <?php checked($settings['block_admin_notices'] === '1'); ?>>
                                    <span class="toggle-slider"></span>
                                </label>
                                <div class="setting-info">
                            ...
                            <div class="german-shield-setting">
                                <label class="german-shield-toggle">
                                    <input type="checkbox" name="block_plugin_ads" value="1" <?php checked($settings['block_plugin_ads'] === '1'); ?>>
                                    <span class="toggle-slider"></span>
                                </label>
                                <div class="setting-info">
                            ...
                            <div class="german-shield-setting">
                                <label class="german-shield-toggle">
                                    <input type="checkbox" name="block_update_notices" value="1" <?php checked($settings['block_update_notices'] === '1'); ?>>
                                    <span class="toggle-slider"></span>
                                </label>
                                <div class="setting-info">
                            ...
                            <div class="german-shield-setting">
                                <label class="german-shield-toggle">
                                    <input type="checkbox" name="block_review_requests" value="1" <?php checked($settings['block_review_requests'] === '1'); ?>>
                                    <span class="toggle-slider"></span>
                                </label>
                                <div class="setting-info">
                ...
                <!-- Lizenz Tab -->
                <div class="german-shield-tab-content <?php echo $active_tab === 'license' ? 'active' : ''; ?>" id="tab-license">
                    <div class="german-shield-section">
                        ...
                        <div style="background: #fff3cd; padding: 15px; border-radius: 8px; border: 2px solid #ffc107; margin-bottom: 20px;">
                            ...
                            $stats_table_exists = $wpdb->get_var("SHOW TABLES LIKE '{$wpdb->prefix}german_shield_stats'");
                            $stats_count = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}german_shield_stats");
                            $recent_stats = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}german_shield_stats ORDER BY created_at DESC LIMIT 5");
                            ...
                            <?php if ($is_free_active): ?>
                                ...
                                <div style="margin: 15px 0; padding: 15px; background: #fff; border-radius: 6px; border: 1px solid #22D6DD;">
                                    ...
                                    <p style="margin: 8px 0 0 0; color: #646970; font-size: 11px;">
                                        💡 Nutze diesen Key um German Shield auf weiteren Domains zu aktivieren!
                                    </p>
                                </div>
                                <?php endif; ?>
                                <p style="margin: 0 0 20px 0; color: #1d2327; font-size: 14px;">
                                    Du nutzt die kostenlose Version von German Shield mit Basis-Funktionen.
                                </p>
                                <form method="post">
                                    <?php wp_nonce_field('german_shield_settings', 'german_shield_nonce'); ?>
                                    <button type="submit" name="deactivate_free" class="german-shield-btn-danger">
                            ...
                            <?php else: ?>
                                <div style="flex: 1;">
                                    ...
                                    <div style="display: flex; gap: 10px; margin-bottom: 20px; border-bottom: 2px solid #22D6DD;">
                                        <button type="button" id="free-email-tab" class="german-shield-free-tab active" 
                                        ...
                                        <button type="button" id="free-key-tab" class="german-shield-free-tab" 
                                            style="flex: 1; padding: 10px; background: transparent; border: none; border-bottom: 3px solid transparent; cursor: pointer; font-weight: 600; transition: all 0.2s;">
                                            🔑 Mit License-Key
                                        </button>
                                    </div>
                                    ...
                                    <!-- E-Mail Aktivierung -->
                                    <div id="free-email-content" class="german-shield-free-content">
                                        <p style="margin: 0 0 20px 0; color: #1d2327; font-size: 14px;">
                                            Gib deine E-Mail-Adresse ein und bestätige sie, um German Shield kostenlos zu nutzen!
                                        </p>
                                    ...
                                    <div style="margin-bottom: 20px;">
                                        <label style="display: flex; align-items: flex-start; gap: 10px; cursor: pointer;">
                                            ...
                                            <span style="font-size: 13px; color: #1d2327;">
                                                Ich akzeptiere die <a href="<?php echo admin_url('admin.php?page=german-shield&show=agb'); ?>" style="color: #22D6DD; text-decoration: underline;">AGB</a> und die <a href="<?php echo admin_url('admin.php?page=german-shield&show=datenschutz'); ?>" style="color: #22D6DD; text-decoration: underline;">Datenschutzerklärung</a>
                                            </span>
                                        </label>
                                    </div>
                                        
                                        <div style="text-align: center; margin-top: auto;">
                                            <button type="button" id="register-free-btn" class="german-shield-btn-primary">
                                    ...
                                    <!-- Key Aktivierung -->
                                    <div id="free-key-content" class="german-shield-free-content" style="display: none;">
                                        <p style="margin: 0 0 20px 0; color: #1d2327; font-size: 14px;">
                                        ...
                                        <div style="text-align: center; margin-top: auto;">
                                            <button type="button" id="activate-free-key-btn" class="german-shield-btn-primary">
                                        ...
                                        <p style="margin: 15px 0 0 0; color: #646970; font-size: 12px; text-align: center;">
                                            💡 Den Key erhältst du nach der E-Mail-Verifizierung. Nutze ihn um German Shield auf weiteren Domains zu aktivieren!
                                        </p>
                                    </div>
                                </div>
                            <?php endif; ?>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                            ...
                            <div style="background: #F2F5F8; padding: 20px; border-radius: 6px; border: 1px solid #c3cbd5; display: flex; flex-direction: column; min-height: 280px;">
                                <?php if ($license_info['has_license'] && $license_status['is_valid']): ?>
                                    ...
                                    <p style="color: #50575e; margin: 0 0 10px 0; font-size: 14px;">
                                        Deine German Shield Lizenz ist aktiv und gültig.
                                    </p>
                                    ...
                                    <form method="post">
                                        <button type="submit" name="deactivate_license" class="german-shield-btn-danger">
                                            Lizenz deaktivieren
                                        </button>
                                    </form>
                                    </div>
                                <?php else: ?>
                                    ...
                                    <form method="post" id="premium-license-form" style="display: flex; flex-direction: column; flex: 1;">
                                        ...
                                        <div style="text-align: center; margin-top: auto;">
                                        <button type="submit" name="activate_license" id="activate-premium-btn" class="german-shield-btn-primary">
                            ...
                            <!-- Box 2: Lizenz kaufen -->
                            <div style="background: #F2F5F8; padding: 20px; border-radius: 6px; border: 1px solid #c3cbd5; display: flex; flex-direction: column; min-height: 280px;">
                                <h3 style="margin: 0 0 15px 0; color: #1d2327; font-size: 16px; font-weight: 600;">💡 German Shield PRO Lizenz</h3>
                                <p style="margin: 0 0 15px 0; color: #50575e; font-size: 14px; flex: 1;">
                                    Noch keine Lizenz? Sichere dir jetzt German Shield Premium und nutze alle Features!
                                </p>
                                ...
                                <div style="text-align: center; margin-top: auto;">
                                <a href="https://german-shield.de/preise" target="_blank" class="german-shield-btn-primary" style="display: inline-flex; text-decoration: none;">
                                    Jetzt kaufen →
                                </a>
                                </div>
                            </div>
                ...
                <!-- Debug Log Tab -->
                <div class="german-shield-tab-content <?php echo $active_tab === 'debug' ? 'active' : ''; ?>" id="tab-debug">
                    <div class="german-shield-section">
                        ...
                        <div style="background: #d1ecf1; padding: 20px; border-radius: 6px; margin-bottom: 20px; border-left: 4px solid #0c5460;">
                            ...
                            <pre style="background: #ffffff; padding: 15px; border-radius: 4px; overflow-x: auto; font-size: 11px; max-height: 500px; font-family: 'Courier New', monospace;"><?php 
                                echo esc_html(German_Shield_Logger::get_log()); 
                            ?></pre>
                            <button type="button" class="german-shield-btn-danger" id="clear-debug-log" style="margin-top: 10px;">
                        ...
                        <div style="background: #F2F5F8; padding: 20px; border-radius: 6px; margin-bottom: 20px;">
                            ...
                            <table class="german-shield-table">
                                ...
                                <tr>
                                    ...
                                    <td><?php echo isset($_POST['german_shield_save_settings']) ? '✅ JA' : '❌ NEIN'; ?></td>
                                </tr>
                                <tr>
                                    <td><strong>Nonce vorhanden:</strong></td>
                                    <td><?php echo isset($_POST['german_shield_nonce']) ? '✅ JA' : '❌ NEIN'; ?></td>
                                </tr>
                                <tr>
                                    <td><strong>Nonce gültig:</strong></td>
                                    <td><?php 
                                        if (isset($_POST['german_shield_nonce'])) {
                                            echo wp_verify_nonce($_POST['german_shield_nonce'], 'german_shield_settings') ? '✅ JA' : '❌ NEIN (FEHLER!)';
                                        } else {
                        ...
                        <div style="background: rgba(34, 214, 221, 0.1); padding: 20px; border-radius: 6px; margin-bottom: 20px; border-left: 4px solid #22D6DD;">
                            ...
                            <pre style="background: #ffffff; padding: 15px; border-radius: 4px; overflow-x: auto; font-size: 12px; max-height: 300px;"><?php 
                                $post_data = $_POST;
                                unset($post_data['german_shield_nonce']);
                        ...
                        <div style="background: #F2F5F8; padding: 20px; border-radius: 6px;">
                            ...
                            <table class="german-shield-table">
                                ...
                                <tr>
                                    ...
                                    <td><?php echo GERMAN_SHIELD_VERSION; ?></td>
                                </tr>
                                <tr>
                                    <td><strong>Admin URL:</strong></td>
                                    <td><?php echo admin_url('admin.php?page=german-shield'); ?></td>
                                </tr>
                                <tr>
                                    <td><strong>Form Action:</strong></td>
                                    <td><?php echo esc_url(admin_url('admin.php?page=german-shield')); ?></td>
                                </tr>
                                <tr>
                ...
                <!-- Einstellungen Tab -->
                <div class="german-shield-tab-content <?php echo $active_tab === 'settings' ? 'active' : ''; ?>" id="tab-settings">
                    <?php if (!$is_free_active && !$license_info['is_valid']): ?>
                        <div style="background: linear-gradient(135deg, rgba(239, 68, 68, 0.1) 0%, rgba(240, 98, 146, 0.1) 100%); padding: 40px; border-radius: 8px; border: 2px solid #ef4444; text-align: center; margin: 20px;">
                            <span style="font-size: 64px;">🔒</span>
                            <h2 style="margin: 20px 0 10px 0; color: #ef4444;">Plugin nicht aktiviert</h2>
                    ...
                    <div class="german-shield-section">
                        ...
                        <div class="german-shield-setting">
                            <label class="german-shield-toggle">
                                <input type="checkbox" name="badge_enabled" value="1" <?php checked($settings['badge_enabled'] === '1'); ?>>
                                <span class="toggle-slider"></span>
                            </label>
                            <div class="setting-info">
                                <h3>Badge anzeigen</h3>
                                <p>Zeigt einen Badge auf der Website, dass sie durch German Shield geschützt wird.</p>
                            </div>
                        </div>
                        
                        <div class="german-shield-subsetting" id="badge-settings" style="<?php echo $settings['badge_enabled'] !== '1' ? 'display:none;' : ''; ?>">
                            ...
                            <div class="setting-row" style="margin-bottom: 20px;">
                                ...
                                <input type="text" name="badge_text" value="<?php echo esc_attr($settings['badge_text'] ?? 'Geschützt durch German Shield'); ?>" 
                                    style="width: 100%; max-width: 500px; padding: 10px; border: 1px solid #c3cbd5; border-radius: 6px; font-size: 14px;"
                                    placeholder="Geschützt durch German Shield">
                            </div>
                            ...
                            <div style="background: #F2F5F8; padding: 20px; border-radius: 6px; text-align: center;">
                                <div id="badge-preview" style="display: inline-flex; align-items: center; gap: 8px; background: #ffffff; padding: 10px 16px; border-radius: 6px; border: 1px solid #22D6DD; box-shadow: 0 2px 8px rgba(34, 214, 221, 0.2);">
                                    <span id="badge-icon">
                                        <img src="<?php echo GERMAN_SHIELD_PLUGIN_URL . 'assets/images/logo_klein.png'; ?>" alt="German Shield" style="width: 24px; height: 24px; object-fit: contain;">
                                    </span>
                                    <span id="badge-text-preview" style="font-size: 13px; font-weight: 600; color: #1d2327;"><?php echo esc_html($settings['badge_text'] ?? 'Geschützt durch German Shield'); ?></span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="german-shield-footer">
                    <div style="text-align: center; padding: 20px; color: #646970; font-size: 13px;">
                        <div style="margin-bottom: 15px; font-weight: 600; color: #22D6DD;">
                            German Shield v0.93 Beta by GermanProWeb
                        </div>
                        <div style="font-size: 12px;">
                            <a href="<?php echo admin_url('admin.php?page=german-shield&show=agb'); ?>" style="color: #646970; text-decoration: none; margin: 0 10px;">AGB</a>
                            <span style="color: #c3cbd5;">|</span>
                            <a href="<?php echo admin_url('admin.php?page=german-shield&show=datenschutz'); ?>" style="color: #646970; text-decoration: none; margin: 0 10px;">Datenschutz</a>
                            <span style="color: #c3cbd5;">|</span>
                            <a href="<?php echo admin_url('admin.php?page=german-shield&show=impressum'); ?>" style="color: #646970; text-decoration: none; margin: 0 10px;">Impressum</a>
                        </div>
                    </div>
                </div>
            </form>
    ...
    private function save_settings() {
        German_Shield_Logger::log_save('save_settings() wird aufgerufen');
        ...
        $settings = array(
            ...
            'badge_text' => sanitize_text_field($_POST['badge_text'] ?? 'Geschützt durch German Shield'),
        ...
        German_Shield_Logger::log_save('Einstellungen vorbereitet', $settings);
        
        $result = update_option('german_shield_settings', $settings);
        
        if ($result) {
            German_Shield_Logger::log_save('update_option() erfolgreich');
        } else {
            German_Shield_Logger::log_error('update_option() fehlgeschlagen oder keine Änderung');
        }
        
        // Verify
        $saved = get_option('german_shield_settings', array());
        German_Shield_Logger::log_save('Gespeicherte Einstellungen verifiziert', array('count' => count($saved)));
    }
