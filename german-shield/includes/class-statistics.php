...
class German_Shield_Statistics {
    
    private $table_name;
    
    public function __construct() {
        global $wpdb;
        $this->table_name = $wpdb->prefix . 'german_shield_stats';
    }
    ...
    public function log_block($type, $ip, $reason, $country = null, $form_id = null, $form_data = null) {
        ...
        German_Shield_Logger::log('[STATS] 🚫 log_block() aufgerufen - Type: ' . $type . ', IP: ' . $ip);
        ...
        if ($result === false) {
            German_Shield_Logger::log('[STATS] ❌ DB INSERT FEHLER: ' . $wpdb->last_error);
        } else {
            German_Shield_Logger::log('[STATS] ✅ Blockierung geloggt, Insert ID: ' . $wpdb->insert_id);
        }
    ...
    private function collect_form_data() {
        ...
        $excluded_keys = array('password', 'pwd', 'pass', 'gs_js_token', 'gs_nonce', 'gs_timestamp', 'gs_honeypot', 'german_shield_nonce');
    ...
    public function log_legitimate($ip, $form_id = null, $form_data = null) {
        ...
        German_Shield_Logger::log('[STATS] ✅ log_legitimate() aufgerufen - IP: ' . $ip);
        ...
        German_Shield_Logger::log('[STATS] Versuche legitime Anfrage einzufügen - Country: ' . $country . ', Form Data: ' . ($form_data ? 'vorhanden' : 'leer'));
        ...
        if ($result === false) {
            ...
            German_Shield_Logger::log('[STATS] ✅ Legitime Anfrage geloggt, Insert ID: ' . $wpdb->insert_id);
        }
    ...
    private function increment_counter($key) {
        $current = get_transient('german_shield_' . $key);
        
        if ($current === false) {
            $current = 0;
        }
        
        set_transient('german_shield_' . $key, $current + 1, DAY_IN_SECONDS);
    }
    ...
    public function clear_all_stats() {
        ...
        // Clear transients
        delete_transient('german_shield_blocks_today');
        delete_transient('german_shield_legitimate_today');
    }
...
add_action('wp_ajax_german_shield_get_stats', function() {
    $stats = new German_Shield_Statistics();
