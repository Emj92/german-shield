...
class German_Shield_Updater {
    ...
    public function __construct() {
        $this->version = GERMAN_SHIELD_VERSION;
        $this->plugin_file = GERMAN_SHIELD_PLUGIN_FILE;
    ...
    private function get_update_info() {
        // Cache prüfen
        $cached = get_transient('german_shield_update_check');
        ...
        set_transient('german_shield_update_check', $data, 12 * HOUR_IN_SECONDS);
    ...
    private function perform_update($update_info) {
        ...
        if ($result) {
            ...
            update_option('german_shield_version', $update_info['version']);
            
            // Run update routines
            $this->run_update_routines($this->version, $update_info['version']);
            
            // Clear cache
            delete_transient('german_shield_update_check');
            wp_cache_flush();
    ...
    private function backup_plugin() {
        ...
        if (class_exists('ZipArchive')) {
            ...
            if ($zip->open($backup_file, ZipArchive::CREATE) === true) {
                $this->add_directory_to_zip($zip, GERMAN_SHIELD_PLUGIN_DIR, '');
                $zip->close();
    ...
    public function post_install($response, $hook_extra, $result) {
        ...
        if (isset($hook_extra['plugin']) && $hook_extra['plugin'] === plugin_basename($this->plugin_file)) {
            $old_version = get_option('german_shield_version', $this->version);
            $new_version = $this->version;
            
            $this->run_update_routines($old_version, $new_version);
            update_option('german_shield_version', $new_version);
        }
...
if (defined('WP_CLI') && WP_CLI) {
     ...
     */
    class German_Shield_CLI_Command {
        ...
        public function __invoke($args, $assoc_args) {
            ...
            WP_CLI::line('Aktuelle Version: ' . GERMAN_SHIELD_VERSION);
    ...
    WP_CLI::add_command('update-p', 'German_Shield_CLI_Command');
}
