...
class German_Shield_AntiSpam {
    ...
    public function enqueue_frontend_scripts() {
        German_Shield_Logger::log('[FRONTEND] enqueue_frontend_scripts() aufgerufen - is_admin: ' . (is_admin() ? 'JA' : 'NEIN'));
        
        if (!$this->should_protect_page()) {
            German_Shield_Logger::log('[FRONTEND] should_protect_page() = false, Scripts werden NICHT geladen');
            return;
        }
        
        German_Shield_Logger::log('[FRONTEND] ✅ Scripts werden geladen');
        ...
        wp_enqueue_script(
            'german-shield-frontend',
            GERMAN_SHIELD_PLUGIN_URL . 'assets/js/frontend.js',
            array('jquery'),
        ...
        if ($defer) {
            add_filter('script_loader_tag', function($tag, $handle) {
                if ($handle === 'german-shield-frontend') {
        ...
        wp_localize_script('german-shield-frontend', 'germanShield', array(
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'timestamp' => time(),
            'honeypot' => $this->generate_honeypot_field_name(),
            'sessionToken' => $this->generate_session_token(),
            'nonce' => wp_create_nonce('german_shield_nonce'),
        ));
        
        wp_enqueue_style(
            'german-shield-frontend',
            GERMAN_SHIELD_PLUGIN_URL . 'assets/css/frontend.css',
            array(),
            GERMAN_SHIELD_VERSION
        );
    }
    ...
    private function generate_honeypot_field_name() {
        $hour = date('YmdH');
        $cached_key = 'german_shield_honeypot_' . $hour;
    ...
    public function check_honeypot($data) {
        ...
        $current_honeypot = get_transient('german_shield_honeypot_' . $current_hour);
        $previous_honeypot = get_transient('german_shield_honeypot_' . $previous_hour);
    ...
    public function add_nonce_field() {
        return wp_nonce_field('german_shield_nonce', 'gs_nonce', true, false);
    }
