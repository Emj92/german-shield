...
class German_Shield_License {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_german_shield_activate_premium', array($this, 'ajax_activate_premium'));
        add_action('wp_ajax_german_shield_deactivate_premium', array($this, 'ajax_deactivate_premium'));
    }
    ...
    public function activate_license($license_key) {
        German_Shield_Logger::log('[LICENSE] Aktiviere Lizenz: ' . substr($license_key, 0, 10) . '...');
        ...
        if (in_array($license_key, $valid_keys)) {
            // Lizenz speichern
            update_option('german_shield_license', array(
                'key' => $license_key,
                'status' => 'active',
            ...
            German_Shield_Logger::log('[LICENSE] Lizenz erfolgreich aktiviert (lokal)');
        ...
        $response = wp_remote_post($this->api_url . '/license/activate', array(
            'body' => json_encode(array(
                ...
                'plugin_version' => GERMAN_SHIELD_VERSION,
            )),
            'headers' => array(
        ...
        if (!is_wp_error($response)) {
            ...
            if (!empty($body['success'])) {
                ...
                German_Shield_Logger::log('[LICENSE] Lizenz erfolgreich aktiviert (API)');
    ...
    public function deactivate_license() {
        $license = get_option('german_shield_license', array());
        ...
        German_Shield_Logger::log('[LICENSE] Deaktiviere Lizenz');
        ...
        delete_option('german_shield_license');
    ...
    public function show_license_notice() {
        ...
        if ($screen->id !== 'toplevel_page_german-shield') {
            return;
        }
        ...
        if (!$license_status['is_valid'] && !$is_free_active) {
            ...
            <div class="notice notice-warning">
                <p>
                    ...
                    <a href="<?php echo admin_url('admin.php?page=german-shield&tab=license'); ?>">
                        Jetzt aktivieren
                    </a>
                </p>
            </div>
    ...
    public function get_license_info() {
        ...
        German_Shield_Logger::log('[LICENSE-INFO] Premium valid: ' . ($status['is_valid'] ? 'JA' : 'NEIN') . ', Free active: ' . ($is_free_active ? 'JA' : 'NEIN') . ', Final valid: ' . ($is_valid ? 'JA' : 'NEIN'));
