...
class German_Shield_Security {
    ...
    public function verify_plugin_integrity() {
        ...
        $last_check = get_transient('german_shield_integrity_check');
        if ($last_check !== false) {
            return;
        }
        
        $current_hash = $this->calculate_integrity_hash();
        $stored_hash = get_option('german_shield_integrity_hash');
        
        // Beim ersten Mal den Hash speichern
        if (!$stored_hash) {
            update_option('german_shield_integrity_hash', $current_hash);
            set_transient('german_shield_integrity_check', true, DAY_IN_SECONDS);
            return;
        }
    ...
    private function calculate_integrity_hash() {
        $files_to_check = array(
            GERMAN_SHIELD_PLUGIN_DIR . 'german-shield.php',
            GERMAN_SHIELD_PLUGIN_DIR . 'includes/class-german-shield.php',
            GERMAN_SHIELD_PLUGIN_DIR . 'includes/class-antispam.php',
            GERMAN_SHIELD_PLUGIN_DIR . 'includes/class-geo-blocking.php',
        );
    ...
    public static function obfuscate($data) {
        ...
        $key = 'GS-' . GERMAN_SHIELD_VERSION . '-MW';
        $result = '';
        $data = (string) $data;
    ...
    private function log_security_event($event_type, $details = '') {
        global $wpdb;
        $table = $wpdb->prefix . 'german_shield_stats';
...
// Initialize Security
German_Shield_Security::get_instance();
